...
To finish this project, and sum up all results, a table is used to present all the important regression-based-models and their performances.

FNN2, FNN3, RNN2, Bidirectional LSTM with attention, transformer4 aswell as transformer5 were the chosen models. They were chosen so that all major different architectures were covered. The strongest performing models (of the ones considered in this project) were chosen for each architecture.
...
The p-values used are two-sided one-sample t-test. They were calculated using the list of 5 benchmarks computed in the 5 trainings performed for each model.

The null-hypthesis is defined as benchmark value 1, the performance of the baseline model.
...
The \href{https://github.com/AntonStantan/matura/blob/main/get_p-value.py}{p-value calculation script} calculates the p-values correctly by applying a logarithmic transformation on the individual benchmarks first.
...
The p-value is a velue between $[0,1]$. It is the probability of observing my benchmark results or more extreme if the model being evaluated performs the same as the baseline model.
