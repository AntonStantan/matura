...
\\[-0.5em]
{\large Betreuer: Michael Schneider, Zweitbetreuerin: Julia Smits}}
\author{Anton Mukin}
\date{June 2025}
...
To define the training data, the decision was made to exclusively use subtraction and addition. The arithmetic expressions were defined to consist of two operators (+ or -) and three integers within the range $[-5, 5]$. The selction of the range for the integers was inspired by \cite{trask2018neuralarithmeticlogicunits}.
...
\]
In total, there are 1907 such expressions in the training data. It can be argued that this is too little data for training a model for this task, as suggested by \cite{baka}. The counter argument is that \cite{trask2018neuralarithmeticlogicunits} used a similar, slightly larger sample size. Additionally, it is later shown in \href{https://github.com/AntonStantan/matura/blob/main/documentation/findings/findings.pdf}{the findings} that the models are able to predict data validation data with a low error rate.
\\[2em]
...
\begin{itemize}
    \item Inside of the number range (1457 expressions): Expressions identical in structure to the training data but not present in the training dataset. 
    \begin{itemize}
        \item[$\qquad\hookrightarrow$]This category ensures the modes is not just memorizing expressions and tests the models interpolation on previously unseen expressions.
    \end{itemize}
    \item Outside of the number range (2048 expressions): The same expression structures, but with numbers in the ranges $[-8, -5]$ and $[5, 8]$, e.g.,
\[
-6 + 6 + 8 \qquad -7 + 6 + 7 \qquad 5 - -6 - 6
\]
    \begin{itemize}
        \item[$\qquad\hookrightarrow$]This category tests the model's extrapolation on data slightly beyond what it encountered in training.
    \end{itemize}
    \item Longer expressions (700 expressions): Expressions of different lengths. Specifically, there are between 2 and 8 numbers inside of the $[-5, 5]$ range, e.g.,
\[
-5 + 1 \qquad 3 + -1 + 4 + -2 + -4 \qquad -2 + 1 + 5 + -5 - -2 + -1 - 2 - 5
\]
    \begin{itemize}
        \item[$\qquad\hookrightarrow$]This category tests the model for structural generalization and it's ability to adapt to sequential, longer, but structurally the same data.
    \end{itemize}
\end{itemize}
...
\\[2em]
During training, later on, the model will require a small validation dataset (636 expressions), which will be used to evaluate the model during training on data not in the training dataset. Based on the model's performance on this datset, it will be either rewarded or punished.
...
The goal of the benchmark, is to compute a model's performance, with a strong emphasis on the model's generalization abilities.
...
\begin{itemize}
    ...
    \item[-] A relative Mean Squared Error (MSE) of the test data with numbers outside of the range.
\end{itemize}
...
\begin{description}
    \item[$\cdot$] For longer expressions, only expressions with 4 numbers were used.
    \item[$\cdot$] For expressions with numbers outside the range, the expression was only used if the absolute values of all three numbers added up to 22.
\end{description}
...
\footnotetext{The number of neurons and layers was chosen, so that the model is capable of predicting with a reasonably good performance on the validation data. (This was done in \href{https://github.com/AntonStantan/matura/blob/main/FNN/FNN1.ipynb}{FNN1.ipynb}.) The exact number of neurons and layers does not really matter, because all models whose benchmark is being evaluated are compared to the same baseline model.}
...
This section corresponds to the \href{https://github.com/AntonStantan/matura/tree/main/FNN}{/FNN directory} in the GitHub repository.

Some notable notebooks here are: \href{https://github.com/AntonStantan/matura/blob/main/FNN/FNN1.ipynb}{FNN1}, \href{https://github.com/AntonStantan/matura/blob/main/FNN/FNN2.ipynb}{FNN2} and \href{https://github.com/AntonStantan/matura/blob/main/FNN/FNN3.ipynb}{FNN3}.
...
\\[1.5em]
To begin, the necessary libraries are imported. Most models in this project, except for the fine-tuned models, use TensorFlow as opposed to PyTorch. 
This is in part because the author was taught to use tensorflow, by \href{https://www.kaggle.com/learn/intermediate-machine-learning}{kaggle tutorials} and in part because of TensorFlows's wider use in the industry and production-ready ecosystem accordig to \cite{alawi2025comparativesurveypytorchvs}.
...
The Adam optimizer (first presented by \cite{kingma2017adammethodstochasticoptimization}) was chosen for most models in this project, due to it's popularity, wide usage and promising results as shown by \cite{ruder2017overviewgradientdescentoptimization}
MSE was chosen because it was used by \cite{trask2018neuralarithmeticlogicunits}, but Mean Absolute Error (MAE) would've also worked.
...
\subsection{FNN3 With Positional Encoding and FNN4}
...
\\
The \href{https://github.com/AntonStantan/matura/blob/main/FNN/FNN4.ipynb}{FNN4} notebooks use a different tokenizer and define the "+" and "-" operators as non-integer floats. They show once and for all that replacing "+" and "-" with integers is not problematic.
...
A couple of additional minor adjustments were made to the model before applying the KerasTuner, mainly switching to the AdamW optimizer, because it is better than Adam for transformers according to \cite{loshchilov2019decoupledweightdecayregularization}. A learning rate scheduler with warmup and cosine decay was also used. This means that, unlike before when the learning rate stayed constant throughout the training, the learning rate now changes. It is higher in the beginning and decays towards the end. This assists the model in taking larger optimization steps initially, thereby overcoming local minima, before proceeding with more precise, smaller steps towards the end of the training.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\paperwidth]{images/learningRate.png}
    \caption{This is how the learning rate could look like for a model being trained over a period of 200 epochs when using a cosine decay with a linear warmup, as in this project. Except for the changing peak learning rate, all other parameters are the same as the ones used in this project.}
...
\begin{itemize}
    ...
    \item The number of dimensions vectors have inside the model
...
The first model that was fine-tuned was Gemini 2.5 Pro. This model was selected partly because of the author's familiarity with it and its high ranking in the \href{https://lmarena.ai/leaderboard}{LMArena}.
...
\subsection{Gemma 3}
For diversity, a small (Gemma3 1B parameters by \cite{gemma1b_2025}) and a tiny (Gemma3 270M parameters by \cite{gemma270m_2025}) model were chosen. They have been provided by Google on Huggingface.
...
Fine-tuning notebooks for Gemma3 models can be found under: \href{https://github.com/AntonStantan/matura/blob/main/pre-trained-tranformers/big_gemma_huggingface.ipynb}{Gemma3 1B}, \href{https://github.com/AntonStantan/matura/blob/main/pre-trained-tranformers/gemma_huggingface.ipynb}{Gemma3 270M}
\\[2em]
...
\begin{itemize}
    ...
    \item \textbf{Data Generation Error:} The second hypothesis suggested an error in the data generation process, such as using different hyperparameters (e.g., temperature) between training and evaluation.
        \begin{itemize}
    ...
    \item \textbf{Improper Weight Loading:} The final explanation could be that the model's weights are not being loaded into the model properly. This was the case when working on FNN2 once.
        \begin{itemize}
...
FNN2, FNN3, RNN2, Bidirectional LSTM with attention, transformer4, as well as transformer5 were the chosen models. They were chosen so that all major different architectures were covered. The strongest performing models\footnote{The best performing models were determined with Keras-Tuner.} were chosen for each architecture.
All of them were evaluated on:
\begin{itemize}
    \item Mean Absolute Error (MAE) on all three of the test datasets separately
    \item Mean Relative Error (MRE) on the test datasets, excluding the one with longer expressions
    \item The Benchmark score
    \item p-value of the Benchmark test
\end{itemize}
...
\subsection{p-Value}
The p-value was calculated using the \href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.ttest_1samp.html}{scipy-stats library}.

The p-values used are two-sided one-sample t-test\footnotemark as first described by \cite{87464e34-37bc-3288-807c-e421e5a0d7a6}. They were calculated using the list of 5 benchmarks computed in the 5 trainings performed for each model.
\footnotetext{A t-test was chosen over a gaussian normality curve because of the small sample size as suggested by \cite{de2013students}.}
...
\end{Verbatim}
In the notebooks, the p-value was computed with a slight inaccuracy (it also accounted for negative benchmarks which are not possible by definition.)
The \href{https://github.com/AntonStantan/matura/blob/main/get_p-value.py}{p-value calculation script} (seen above) calculates the p-values correctly by applying a logarithmic transformation on the individual benchmarks first.
This is necessary because the benchmark can only be positive by defintion and a two-sided t-test also accounts for negative numbers. The logarithmic transformation converts values between $(0,1)$ to be negative. It also changes baseline benchmark: $ln(1) = 0$
...
The performance of the Small and Large Language Models was evaluated with accuracy, not deviation, because these types of models generate discrete tokens for each number in an expression. For them, solving an expression isn't a regression task, but a classification task. This means the output of these models will always be an integer, never a float like with the regression models. Compared to the solution of an expression the model's output will either be right or wrong.
\\[1em]
...
The results of the questionnaire along with the author's comments are available \href{https://github.com/AntonStantan/matura/tree/main/documentation/Questionnaire.pdf}{on github}.
...
It was a very interesting project and a pleasure to work on.
If you also find this project interesting, please consider starring the GitHub repository.
\\[1em]
The author would like to thank Herr Schneider for his valuable insights and prompt responses, as well as Frau Smits for her availabilty to help. 
