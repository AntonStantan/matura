...
The p-values\footnote{two-sided one-sample t-test} calculated symbolize the propability that the benchmark will be $\le 1$ (or $\ge 1$ if the model's benchmark is smaller than 1, which is the case for RNN2). 
for this project, the small p-values ($<0.05$) mean that the models will perform better than the baseline model (or respectively worse in the case of RNN2). This proves the significance of the results.

The fine-tuned Language Models were also evaluated, but on their accuracy:
\\[0.5em]
\begin{table}[htbp]
\centering
\footnotesize
