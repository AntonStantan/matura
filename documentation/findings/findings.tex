...
To refresh your memory for the next subsection, imagine the weights as the 'strengths' of connections between neurons and biases as 'small nudges' which are applied to the neurons throughout a neural network.
\subsection{Backpropagation}
Backpropagation describes the method, used by neural networks in training, to tweak the weights and biases of different neurons. Imagine a FNN, like the one in the image below.
\\[1em]
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{images/BackpropagationFNN.png}
    \caption{Backpropagation in a FNN for regression as described by \cite{geeksforgeeks_backprop}.}
    \label{fig:backprop}
\end{figure}

The method consists of two phases. First the data is fed forward through the neural network to obtain the output. The output is compared to the matching true result and an Error is calculated.
In the next and final phase, the search for who is to blame commences. For example, imagine the weight from the neuron h(2,1) to the output neuron is far greater, than the weights from h(2,2) and h(2,3). In this case, it is obvious that the h(2,1) neuron should be held accountable for the error, more than the other 2 neurons. This is done by 'punishing' the weights of neuron h(2,1).
Using this principle, the model can feed backwards through the network and reassign the neurons with adjusted weights and biases.
\newpage
To make this more apparent some equations are formulated. Let:
\begin{itemize}
    \item $x_j$ is the output of neuron $j$
    \item $f$ is the activation function (in our case: PReLU)
    \item $w_{ji}$ is the weight connecting input $i$ (of the previous layer) to the neuron $j$
    \item $b_j$ is the bias of neuron $j$
    \item $L$ is the loss function, here and in our case: MSE
    \item $y$ is the true answer corresponding to the predicted answer $x_o$
    \item $z_j$ is the pre-activation output of neuron $j$
    \item $\delta_j$ is the error signal for neuron $j$
    \item $\eta$ is the learning rate
\end{itemize}


The forward pass is defined as the following equations. The equation \eqref{eq:lit} is taken from the literature study.
\begin{gather}
x_j = f\left(\sum_i (w_{ji} x_i) + b_j\right) \label{eq:lit} \\
L = (y - x_o)^2 \\
z_j = \sum_i (w_{ji} x_i) + b_j
\end{gather}
In the second phase of the method, the model propagates the error signal (defined in equation \eqref{eq:error_signal}) backwards, by multiplying it with a previously defined learning rate and subtracting the individual products from all weights and biases in equations: \eqref{eq:weight_new}, \eqref{eq:bias_new}
\begin{gather}
\delta_j = \frac{\partial L}{\partial z_j} \label{eq:error_signal} \\
w_{ji}^{\text{new}} = w_{ji}^{\text{old}} - \eta \delta_j \label{eq:weight_new} \\
b_{j}^{\text{new}} = b_{j}^{\text{old}} - \eta \delta_j \label{eq:bias_new}
\end{gather}

Sources from which the equations are drawn: \cite{Goodfellow-et-al-2016} and \cite{nielsen2015neural}.
...
\\[2em]
An attention score is calculated for each timestep in \eqref{eq:v_bahdanau}, \eqref{eq:attention_score} and it is then normalized in \eqref{eq:alpha_score}. Finally the attention weights scale their respective hidden-states from the LSTMs, to compute the context vector \eqref{eq:attention_context}.
...
\section{Findings}
Lastly, the most noteworthy architectures were evaluated and scored on a benchmark. A custom benchmark was defined for this rather unique project, so that it takes into account the different generalization capabilities of models. For details, see \href{https://github.com/AntonStantan/matura/blob/main/documentation/methodology/methodology.pdf}{the methodology}, section: 2.3 The Benchmark. 
...
\multicolumn{2}{|l|}{\cellcolor[HTML]{B5E6A2}Benchmark   score p-value:} & \multicolumn{1}{r|}{\cellcolor[HTML]{B5E6A2}0.000} & \multicolumn{1}{r|}{\cellcolor[HTML]{B5E6A2}0.000} & \multicolumn{1}{r|}{\cellcolor[HTML]{B5E6A2}0.001} & \multicolumn{1}{r|}{\cellcolor[HTML]{B5E6A2}0.147} & \multicolumn{1}{r|}{\cellcolor[HTML]{B5E6A2}0.000} & \multicolumn{1}{r|}{\cellcolor[HTML]{B5E6A2}0.001} \\ \hline
\end{tabular}
\end{table}

\subsection{p-Value}

The p-values\footnote{Two-sided one-sample t-test was calculated on the natural logarithmic transformation of 5 benchmarks.} calculated symbolize the propability that the next benchmark calculated is $\le 1$ (or $\ge 1$ if the model's benchmark is smaller than 1, which is the case for RNN2). 
...
The p-values prove that all models except RNN2 and the Bidirectional LSTM with attention performed significantly better (with a 95\% confidence interval) than the baseline model on the benchmark.
...
\begin{description}
    \item A logical and important continuation of this project is using the benchmark as the loss function; this might lead to the training of better-performing models, as well as a struggle with overfitting. 
    \item The fine-tuned models have been evaluated very sparsly in this study. A possible continuation would be to evaluate them in more detail. (In this study this wasn't done, because of the computational costs.)
    \item The relatively small amount of data samples in the training data could why the neural networks were not able to generalize rules. One possible direction would be to use the same number range, but similar to how \cite{trask2018neuralarithmeticlogicunits} did it, use float-numbers in the training data instead of limiting yourself to only integers. This approax would allow for more training data.
    \item Another possible direction is improving models by evaluating different activation functions, optimizers, or training data and using the best one, or even designing one's own. Training data and its tokenizer and padding is an area with lots of potential for improvement, i.e., by using an additional embedding, or finding the best training data, to teach a neural network simple arithmetics. 
    \item Also, consider this a proposal to evaluate a model architecture: an FNN with attention seems to be promising. Judging by the results of this study, this model is expected to improve the FNN's performance with longer expressions than in training, as well as strengthen its ability to accurately and confidently predict test data inside of the number range, previously referred to as the classical validation data.
    \item A minimal but still interesting branch can be formed from this project to find the error or investigate the exact reason for the accuracy missmatch observed during and after fine-tuning.
...
If you also find this project interesting, please consider starring the \href{https://github.com/AntonStantan/matura/tree/main}{GitHub repository}.
\\[1em]
